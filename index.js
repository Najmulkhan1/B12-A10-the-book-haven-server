    ...
    app.post("/reviews", async (req, res) => {
      try {
        ...
        if (typeof data.rating !== "number") {
          return res
            .status(400)
            .send({ ok: false, message: "rating must be a number" });
        }
        ...
        const bookFilter = tryObjectId
          ? { _id: tryObjectId }
          : { _id: productIdRaw };
        const newAvgRounded = Math.round((stats.avgRating || 0) * 10) / 10; // 1 decimal

        const updateResult = await booksCollection.updateOne(bookFilter, {
          $set: {
        ...
        res
          .status(500)
          .send({ ok: false, message: "Server error", error: err.message });
      }
    });
